			...
			if context.setting_blind then
				for i = 1, #G.jokers.cards do
					if not Talisman.config_file.disable_anims then
						G.E_MANAGER:add_event(Event({
							func = function()
								G.jokers.cards[i]:juice_up(0.5, 0.5)
								card_eval_status_text(card, 'extra', nil, nil, nil, {message = "-5% Blind Size", colour = G.C.DARK_EDITION});
								return true
							end,
						}))
					end
					...
					if pseudorandom("mmj-Daisy") < G.GAME.probabilities.normal/card.ability.extra.odds then
						   G.E_MANAGER:add_event(Event({
							func = (function()
								G.E_MANAGER:add_event(Event({
									func = function() 
										local _card = create_card('Consumeables',G.consumeables, nil, nil, nil, nil, nil, 'car')
										_card:add_to_deck()
										G.consumeables:emplace(_card)
										return true
									end}))   
									card_eval_status_text(card, 'extra', nil, nil, nil, { message = localize("mmj_daisy_give") })                       
								return true
							end)}))	
					else
					G.E_MANAGER:add_event(Event({
...
local moremariojokerTabs = function() return {
	{
      ...
      settings.nodes[#settings.nodes + 1] =
        create_toggle({ label = localize("mmj_config_nostolgic_luigi"), ref_table = mmj_config, ref_value = "Nostolgic_luigi" })
      settings.nodes[#settings.nodes + 1] =
