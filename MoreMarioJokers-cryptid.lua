	...
	SMODS.Joker {
		...
		calculate = function(self, card, context)
			if context.end_of_round and not context.individual and not context.repetition then
				for i = 1, #G.jokers.cards do
				if G.jokers.cards[i]:can_calculate and pseudorandom("rosalina") < G.GAME.probabilities.normal / card.ability.extra.odds then --Code "borrowed" from black hole
					update_hand_text(
				...
				elseif G.jokers.cards[i]:can_calculate then
					G.E_MANAGER:add_event(Event({
						trigger = "after",
						delay = 0.4,
						func = function() --"borrowed" from Wheel Of Fortune
